...
### Ragdoll Mode
Designed for simulating physics using [Verlet Integration](https://en.wikipedia.org/wiki/Verlet_integration). In this mode, you're able to create different elements which model objects that are affected by world forces, such as gravity and friction. The elements are: ```Points, Sticks, IK Chains, Boxes, Elastic Meshes, Solid Meshes and Cloth```.
